    ...
    def _state_change_handler(self, ev):
        ...
        if ev.state == MAIN_DISPATCHER:
            if not datapath.id in self.datapaths:
                ...
                #HAMED Jul 17
                # self.plr_srcid_dstid_port_dict[datapath.id]={}
                self.Graph_plr[datapath.id]={}
                self.PortMap_topo[datapath.id]={}    
                self.PortMap_send[datapath.id]={}    
                self.PortMap_recv[datapath.id]={}
            ...
            try:
                ...
                if len(self.topo_raw_switches) > 0:
                    for s1,s2,sp,dp in mylinks:
                        self.PortMap_topo[s1][s2]=sp
                        self.PortMap_topo[s2][s1]=dp
                        self.PortMap_send[s1][s2]=0
                        self.PortMap_recv[s1][s2]=0
                        self.Graph_plr[s1][s2]=0
                for dp in self.datapaths.values():
                    self._request_stats(dp)                
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                ...
                try:
                    ...
                    if len(self.topo_raw_switches) > 0:
                        for s1,s2,sp,dp in mylinks:
                            if s1==dpid and sp==port_no:
                                self.PortMap_send[s1][s2]=stat.tx_packets 
                                self.PortMap_recv[s1][s2]=stat.rx_packets
                                self.Graph_plr[s1][s2]=self._get_plr(self.PortMap_send[s1][s2],
                                                                    self.PortMap_recv[s2][s1])
                except Exception as inst:     
                    print("Exception=",inst)
                
                # print('LOG >>>>>>>>>>>>>>>>>>>>>>>')
                # print(f'self.PortMap_send: {self.PortMap_send}\n')
                # print(f'self.PortMap_recv: {self.PortMap_recv}\n')
                # print(f'self.PortMap_topo: {self.PortMap_topo}\n')
                # print(f'self.Graph_plr: {self.Graph_plr}\n')
                # print('<<<<<<<<<<<<<<<<<<<<<<<<<<<')

                # self._save_stats(self.port_plr, key, value, 2)
                ...
                #         if s==dpid and sp==port_no:
                #             # if d in self.port_plr.keys[0]:
                #             tmp_tx_key=(s,sp)
                #             tmp_rx_key=(d,dp)
                #             # print('LOG >>>>>>>>>>>>>>>>>>>>>>>')
                #             # print(f'mylinks: {mylinks}\n')
                #             # print('<<<<<<<<<<<<<<<<<<<<<<<<<<<')
                #             tx=self.port_plr[tmp_tx_key][-1][0]
                #             rx=self.port_plr[tmp_rx_key][-1][1]
                #             self.plr_srcid_dstid_port_dict[s][d]=(tx-rx)/(tx)
                # except Exception as inst:     
                #     print("Exception=",inst)
                # print('LOG >>>>>>>>>>>>>>>>>>>>>>>')
                # print(f'self.plr_srcid_dstid_port_dict: {self.plr_srcid_dstid_port_dict}\n')
                # print('<<<<<<<<<<<<<<<<<<<<<<<<<<<')
